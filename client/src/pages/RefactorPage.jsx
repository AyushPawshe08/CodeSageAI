...
const RefactorPage = ({ navigate }) => {
    ...
    const handleSubmit = async ({ code, language, files }) => {
        ...
        setRefactoredCode('')
        try {
            const response = await submitReviewCode({ code, language, files, mode: 'refactor' })
            const history = await getHistoryItem(response.id)
            setStoredId(response.id)
            setRefactoredCode(history.refactored_code)
        } catch (err) {
            setError(err.message || 'Failed to process refactor')
        } finally {
            setLoading(false)
        }
    }
        ...
        <div style={{ display: 'grid', gap: '18px' }}>
            <section>
                ...
                <p style={{ margin: 0, color: '#4b5563', lineHeight: 1.7, maxWidth: '700px' }}>
                    Submit code, let the backend generate a refactor, and focus on the cleaned code only on this page.
                </p>
            </section>
            {error && (
                <div style={{
            ...
            <div style={{
                ...
                <section style={{
                    ...
                    <CodeUploadForm mode="refactor" onSubmit={handleSubmit} loading={loading} />
                </section>
                <ResultPanel
                    title="Refactored code"
                    content={refactoredCode}
                    emptyText={loading ? 'Processing refactor...' : 'Your refactored code will appear here.'}
                />
            </div>
        </div>
    )
